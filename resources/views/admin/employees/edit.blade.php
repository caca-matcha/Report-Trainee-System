<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-widest">{{ $employee->name }}</p>
            </div>
        </div>
    </x-slot>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Foto
                            Trainee</label>
                        <div id="photoPreviewContainer" class="{{ $employee->photo ? '' : 'hidden' }} mb-4">
                            <div class="relative inline-block">
                                <img id="photoPreview" src="{{ $employee->photo ? asset('storage/' . $employee->photo) : '' }}" class="w-32 h-32 rounded-2xl object-cover border-2 border-indigo-500 shadow-md">
                            ...
                            @if($employee->photo)
                                <p id="currentPhotoNote" class="text-[10px] text-gray-400 mt-1 italic">*Foto saat ini</p>
                            @endif
                        </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="npk" value="{{ old('npk', $employee->npk ?? $employee->email) }}"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    ...
                    <div>
                        ...
                        <input type="email" name="email"
                            value="{{ old('email', str_contains($employee->email, '@') ? $employee->email : '') }}"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Sub
                            Company</label>
                        <input type="text" name="subco" value="{{ old('subco', $employee->subco) }}"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    </div>
                </div>
    ...
    <script>
        ...
        const currentPhotoNote = document.getElementById('currentPhotoNote');
        ...
        photoInput.addEventListener('change', function(e) {
            ...
            reader.onload = function(event) {
                ...
                if (currentPhotoNote) currentPhotoNote.classList.add('hidden');
        ...
        function removePhoto() {
            photoInput.value = '';
            // If it was an existing photo, reset it or clear it
            @if($employee->photo)
                photoPreview.src = "{{ asset('storage/' . $employee->photo) }}";
                if (currentPhotoNote) currentPhotoNote.classList.remove('hidden');
            @else
                photoPreview.src = "";
