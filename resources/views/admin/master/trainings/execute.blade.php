<x-admin-layout>
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div class="space-y-1">
                <div class="flex items-center gap-2 mb-2">
                    <a href="{{ route('admin.master-trainings.index') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline text-xs font-bold uppercase tracking-widest flex items-center gap-1">
            ...
            <div class="flex items-center gap-4">
                <div class="px-4 py-2 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 rounded-xl">
                    <div class="text-[10px] text-indigo-500 dark:text-indigo-400 font-black uppercase tracking-widest">No. Training</div>
                    <div class="text-xs font-bold text-indigo-700 dark:text-indigo-300">{{ $masterTraining->event_no }}</div>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        <form action="{{ route('admin.master-trainings.store_execution', $masterTraining) }}" method="POST" id="execution-form">
            @csrf

            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                {{-- Left Column: Configuration Sidebar --}}
                <div class="lg:col-span-4 space-y-8">
                    <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] p-8 shadow-sm border border-gray-100 dark:border-gray-700/50">
                        <div class="flex items-center gap-4 mb-8">
                            <div class="w-10 h-10 rounded-2xl bg-indigo-50 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-600 dark:text-indigo-400">
                                <i data-lucide="settings-2" class="w-5 h-5"></i>
                            </div>
                            <h3 class="text-base font-black text-gray-900 dark:text-white uppercase tracking-wider">Konfigurasi</h3>
                        </div>

                        <div class="space-y-6">
                            <div>
                                <label class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2">Nama Training</label>
                                <div class="px-5 py-3.5 bg-gray-50 dark:bg-gray-900/50 border border-transparent rounded-2xl text-sm font-bold text-gray-900 dark:text-white">
                            ...
                            <div>
                                <label for="training_topic" class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2">Topic / Judul Sesi</label>
                            ...
                            <div>
                                <label for="organizer" class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2">Penyelenggara</label>
                                <input type="text" name="organizer" id="organizer" value="{{ old('organizer', $masterTraining->provider) }}" required
                            ...
                            <div class="space-y-4">
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label for="start_date" class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2">Tgl Mulai</label>
                                        <input type="date" name="start_date" id="start_date" value="{{ old('start_date', now()->format('Y-m-d')) }}" required
                                            class="w-full px-4 py-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl text-xs font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all dark:text-white appearance-none">
                                    </div>
                                    <div>
                                        <label for="end_date" class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2">Tgl Selesai</label>
                                        <input type="date" name="end_date" id="end_date" value="{{ old('end_date', now()->format('Y-m-d')) }}" required
                                            class="w-full px-4 py-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl text-xs font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all dark:text-white appearance-none">
                                    </div>
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2">Estimasi Durasi</label>
                                    <div class="relative">
                                        ...
                                        <i data-lucide="calendar-check" class="absolute right-5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-indigo-500/40"></i>
                                    </div>
                                </div>
                            </div>

                            <div>
                                <label for="passing_grade" class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2">Passing Grade (%)</label>
                                <div class="relative">
                                    <input type="number" name="passing_grade" id="passing_grade" value="{{ old('passing_grade', $masterTraining->passing_grade ?? 70) }}" required
                                    ...
                                    <span class="absolute right-5 inset-y-0 flex items-center text-xs font-black text-gray-400">%</span>
                                </div>
                            </div>

                            <div>
                                <label for="description" class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2">Deskripsi / Catatan</label>
                                <textarea name="description" id="description" rows="2" placeholder="Tambahkan catatan khusus..."
                    ...
                    <div class="bg-indigo-600 rounded-[2.5rem] p-8 shadow-2xl shadow-indigo-500/40 text-white relative overflow-hidden group">
                        ...
                        <div class="relative z-10">
                            ...
                            <button type="submit" class="w-full py-5 bg-white text-indigo-600 rounded-2xl font-black uppercase tracking-widest text-xs hover:shadow-xl hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-3">
                ...
                <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {{-- Trainers Card --}}
                        <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-gray-700/50 flex flex-col h-full">
                            <div class="p-10 border-b border-gray-50 dark:border-gray-700/50">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-sm border border-emerald-100 dark:border-emerald-800">
                                            <i data-lucide="award" class="w-6 h-6"></i>
                                        </div>
                                        <h3 class="text-base font-black text-gray-900 dark:text-white uppercase tracking-wider">Trainers</h3>
                                    </div>
                                    <button type="button" id="btn-add-manual-trainer" class="text-[10px] font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 px-4 py-2 rounded-2xl transition-all border border-emerald-100 dark:border-emerald-800">+ Manual</button>
                                </div>
                                <div class="mt-8 relative group">
                                    ...
                                    <i data-lucide="search" class="absolute left-4.5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-emerald-500 transition-colors"></i>
                                    <div id="quick-search-trainer-suggestions" class="absolute z-50 w-full bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2rem] shadow-2xl mt-4 hidden max-h-72 overflow-y-auto"></div>
                                </div>
                            </div>
                            <div id="trainer_list" class="flex-1 p-8 space-y-4 min-h-[160px]">
                                @php $trainers = old('trainers', $masterTraining->trainers ?? []); @endphp
                                @foreach($trainers as $index => $trainer)
                                    <div class="trainer-row flex items-center gap-4 bg-gray-50 dark:bg-gray-900/40 p-3.5 rounded-2xl border border-gray-100 dark:border-gray-700/30 group hover:border-emerald-200 dark:hover:border-emerald-500/20 transition-all">
                                        ...
                                        <button type="button" class="remove-trainer p-2 text-red-400 hover:text-red-600 transition-colors opacity-0 group-hover:opacity-100">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
                                    </div>
                                @endforeach
                                @if(empty($trainers))
                                    <div class="h-full flex flex-col items-center justify-center py-6 text-gray-400 opacity-40">
                                        <i data-lucide="users-2" class="w-8 h-8 mb-2"></i>
                                        <p class="text-[10px] font-bold uppercase tracking-widest italic">Belum ada trainer</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-gray-700/50 flex flex-col h-full">
                            <div class="p-10 border-b border-gray-50 dark:border-gray-700/50">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 shadow-sm border border-blue-100 dark:border-blue-800">
                                            <i data-lucide="user-cog" class="w-6 h-6"></i>
                                        </div>
                                        <h3 class="text-base font-black text-gray-900 dark:text-white uppercase tracking-wider">PICs</h3>
                                    </div>
                                    <button type="button" id="btn-add-manual-pic" class="text-[10px] font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 px-4 py-2 rounded-2xl transition-all border border-blue-100 dark:border-blue-800">+ Manual</button>
                                </div>
                                <div class="mt-8 relative group">
                                    ...
                                    <i data-lucide="search" class="absolute left-4.5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-blue-500 transition-colors"></i>
                                    <div id="quick-search-pic-suggestions" class="absolute z-50 w-full bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2rem] shadow-2xl mt-4 hidden max-h-72 overflow-y-auto"></div>
                                </div>
                            </div>
                            <div id="pic_list" class="flex-1 p-8 space-y-4 min-h-[160px]">
                                @php $pics = old('pics', $masterTraining->pics ?? []); @endphp
                                @foreach($pics as $index => $pic)
                                    <div class="pic-row flex items-center gap-5 bg-gray-50 dark:bg-gray-900/40 p-4 rounded-2xl border border-gray-100 dark:border-gray-700/30 group hover:border-blue-200 dark:hover:border-blue-500/10 transition-all shadow-sm">
                                        <div class="w-12 h-12 rounded-xl bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center text-blue-600 font-bold shrink-0 shadow-sm">
                                        ...
                                        <button type="button" class="remove-pic p-2.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-xl transition-colors opacity-0 group-hover:opacity-100">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
                                    </div>
                                @endforeach
                                @if(empty($pics))
                                    <div class="h-full flex flex-col items-center justify-center py-10 text-gray-400 opacity-40">
                                        <i data-lucide="user-check" class="w-10 h-10 mb-3"></i>
                                        <p class="text-[10px] font-black uppercase tracking-widest italic text-center">Belum ada PIC</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-700/50 overflow-hidden">
                        <div class="p-10 border-b border-gray-50 dark:border-gray-700/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                            <div class="flex items-center gap-5">
                                <div class="w-14 h-14 rounded-2xl bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shadow-sm border border-indigo-100 dark:border-indigo-800">
                                    <i data-lucide="users" class="w-7 h-7"></i>
                                </div>
                                <div>
                                    <h3 class="text-base font-black text-gray-900 dark:text-white uppercase tracking-wider">Daftar Peserta Pelatihan</h3>
                                    <p class="text-xs text-gray-400 font-bold uppercase tracking-[0.1em] mt-1.5"><span id="participant_count" class="text-indigo-600 dark:text-indigo-400">{{ count($masterTraining->participants ?? []) }}</span> Peserta</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-4 w-full md:w-auto">
                                <button type="button" id="btn-add-manual-participant" class="px-5 py-3 bg-white dark:bg-gray-800 text-indigo-600 dark:text-indigo-400 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all border border-indigo-100 dark:border-indigo-800 whitespace-nowrap flex items-center gap-2">
                                ...
                                <div class="relative flex-1 md:w-80 group">
                                    ...
                                    <i data-lucide="search" class="absolute left-4.5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-indigo-500 transition-colors"></i>
                                    <div id="quick-search-suggestions" class="absolute z-50 w-full bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2rem] shadow-2xl mt-4 hidden max-h-96 overflow-y-auto"></div>
                                </div>
                            </div>
                        </div>

                        <div class="overflow-x-auto">
                            <table class="w-full text-left" id="participant_table">
                                <thead>
                                    <tr class="bg-gray-50/50 dark:bg-gray-900/50 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500">
                                        <th class="px-10 py-6">Identitas Trainee</th>
                                        <th class="px-10 py-6">Unit Kerja / Dept</th>
                                        <th class="px-10 py-6 text-center w-32">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                                    @php $participants = old('participants', $masterTraining->participants ?? []); @endphp
                                    @forelse($participants as $index => $participant)
                                        <tr class="participant-row group hover:bg-indigo-50/40 dark:hover:bg-indigo-900/10 transition-all duration-300">
                                            <td class="px-10 py-7">
                                                <div class="flex flex-col max-w-xs sm:max-w-sm md:max-w-md">
                                                    <span class="text-sm font-black text-gray-900 dark:text-white leading-tight mb-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors break-words">{{ is_array($participant) ? ($participant['name'] ?? '') : $participant->name }}</span>
                                                    <span class="text-[10px] font-black font-mono tracking-tighter text-indigo-500/70 dark:text-indigo-400/50 uppercase italic">{{ is_array($participant) ? ($participant['npk'] ?? '-') : $participant->npk }}</span>
                                                </div>
                                            ...
                                            <td class="px-10 py-7">
                                                <div class="flex items-center gap-3">
                                                    <span class="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-[10px] font-black text-gray-600 dark:text-gray-400 rounded-lg tracking-tighter shadow-sm">{{ is_array($participant) ? ($participant['department'] ?? '-') : $participant->department }}</span>
                                                    <span class="text-gray-200 dark:text-gray-800">|</span>
                                                    <span class="text-[10px] font-bold text-gray-400 dark:text-gray-500 tracking-wider lowercase italic">{{ is_array($participant) ? ($participant['subco'] ?? '-') : $participant->subco }}</span>
                                                </div>
                                            ...
                                            <td class="px-10 py-7 text-center">
                                                <button type="button" class="remove-participant p-3 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-2xl transition-all shadow-sm opacity-0 group-hover:opacity-100">
                                        ...
                                        <tr id="empty_row">
                                            <td colspan="4" class="px-8 py-20 text-center">
                                                <div class="flex flex-col items-center opacity-30">
                                                    <i data-lucide="user-plus" class="w-12 h-12 mb-4"></i>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const participantTable = document.querySelector('#participant_table tbody');
            ...
            function addParticipantRow(user) {
                ...
                const html = `
                    <tr class="participant-row group hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 transition-all duration-300 animate-fade-in">
                        <td class="px-10 py-6">
                            <div class="flex flex-col">
                                <span class="text-sm font-black text-gray-900 dark:text-white leading-tight mb-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">${user.name}</span>
                                <span class="text-[10px] font-black font-mono tracking-tighter text-indigo-500/70 uppercase italic">${user.npk || '-'}</span>
                            </div>
                        ...
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-3">
                                <span class="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-[10px] font-black text-gray-600 dark:text-gray-400 rounded-lg tracking-tighter shadow-sm">${user.department || '-'}</span>
                                <span class="text-gray-200 dark:text-gray-800">|</span>
                                <span class="text-[10px] font-bold text-gray-400 dark:text-gray-500 tracking-wider lowercase">${user.subco || '-'}</span>
                            </div>
                        ...
                        <td class="px-10 py-6 text-center">
                            <button type="button" class="remove-participant p-3 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-2xl transition-all shadow-sm">
                                <svg class="w-5 h-5 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
                        </td>
                    </tr>
                `;
                participantTable.insertAdjacentHTML('beforeend', html);
                participantIndex++;
                updateCount();
            ...
            document.getElementById('btn-add-manual-participant').addEventListener('click', () => {
                ...
                const html = `
                    <tr class="participant-row bg-white dark:bg-gray-800 border-2 border-dashed border-indigo-100 dark:border-indigo-900/20 animate-fade-in">
                        <td class="px-8 py-5">
                            <div class="grid grid-cols-1 gap-2">
                                <input type="text" name="participants[${participantIndex}][name]" placeholder="Nama Lengkap" required
                                    class="w-full bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 px-3 py-2 rounded-xl text-sm font-bold dark:text-white">
                                <input type="text" name="participants[${participantIndex}][npk]" placeholder="NPK (Opsional)"
                                    class="w-full bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 px-3 py-2 rounded-xl text-[10px] font-black font-mono dark:text-white uppercase">
                            </div>
                        </td>
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-2">
                                <input type="text" name="participants[${participantIndex}][department]" placeholder="Dept"
                                    class="w-full bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 px-3 py-2 rounded-xl text-[10px] font-black uppercase dark:text-white">
                                <span class="text-gray-400">/</span>
                                <input type="text" name="participants[${participantIndex}][subco]" placeholder="Subco"
                                    class="w-full bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 px-3 py-2 rounded-xl text-[10px] font-black uppercase dark:text-white">
                            </div>
                        </td>
                        <td class="px-8 py-5 text-center">
                            <button type="button" class="remove-participant p-2.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all">
                                <svg class="w-5 h-5 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
                        </td>
                    </tr>
            ...
            function updateCount() {
                const rows = participantTable.querySelectorAll('.participant-row');
                document.getElementById('participant_count').innerText = rows.length;
                if (rows.length === 0 && !document.getElementById('empty_row')) {
                    participantTable.innerHTML = `
                        <tr id="empty_row" class="animate-fade-in">
                            <td colspan="4" class="px-8 py-20 text-center">
                                <div class="flex flex-col items-center opacity-30">
                                    <i data-lucide="user-plus" class="w-12 h-12 mb-4"></i>
                                    <p class="text-xs font-black uppercase tracking-[0.2em] italic">Belum ada peserta terdaftar</p>
    ...
    <style>
        ...
        .animate-fade-out { animation: fadeOut 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
    </style>
    @endpush
</x-admin-layout>
