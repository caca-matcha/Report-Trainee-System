...
<body class="font-sans antialiased bg-gray-50 dark:bg-gray-900 overflow-hidden transition-colors duration-300">
    <div x-data="{ 
        ...
        {{-- Sidebar --}}
        <aside :class="sidebarMinimized ? 'w-24' : 'w-64'"
            ...
            <div class="px-4 py-4 shrink-0 transition-all duration-300" :class="sidebarMinimized ? 'px-2' : 'px-4'">
                <button @click="toggleSidebar()"
                    ...
                    {{-- Icon Container --}}
                    <div
                        class="flex items-center justify-center w-10 h-10 rounded-xl bg-gray-50 dark:bg-gray-800 group-hover:bg-white dark:group-hover:bg-indigo-600 text-gray-400 dark:text-gray-500 group-hover:text-indigo-600 dark:group-hover:text-white transition-all duration-500 shadow-inner font-black overflow-hidden">
                    ...
                    <div x-show="!sidebarMinimized" x-transition:enter="transition ease-out duration-300 delay-100"
                        ...
                        <span
                            class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors duration-300">
                            View Model
                        </span>
                        <div class="flex items-center gap-1.5">
                            <span
                                class="text-[9px] font-bold text-gray-300 dark:text-gray-700 uppercase tracking-widest leading-none">Compact
                                Mode</span>
                        </div>
                    </div>
                    ...
                    {{-- Hover Bloom Effect --}}
                    <div
                        class="absolute inset-0 bg-indigo-500/5 dark:bg-indigo-500/10 opacity-0 group-hover:opacity-100 rounded-2xl transition-opacity duration-500 pointer-events-none">
                    </div>
                </button>
            </div>
            ...
            <nav class="flex-1 py-6 space-y-1 overflow-y-auto scrollbar-hide transition-all duration-300"
                ...
                <p x-show="!sidebarMinimized" x-transition.opacity
                    class="px-3 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 opacity-50">Main
                    Menu</p>
                ...
                <a href="{{ route('admin.dashboard') }}"
                    ...
                    :class="sidebarMinimized ? 'justify-center h-14 w-full' : 'px-3 py-3.5 gap-4'" title="Dashboard">
                    <svg class="w-6 h-6 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                    </svg>
                    <span x-show="!sidebarMinimized" x-transition.opacity
                        class="truncate font-semibold tracking-wide">Dashboard</span>
                </a>
                ...
                <a href="{{ route('admin.users.index') }}"
                    ...
                    <span x-show="!sidebarMinimized" x-transition.opacity
                        class="truncate font-semibold tracking-wide">Manajemen User</span>
                </a>
                ...
                <a href="{{ route('admin.trainings.index') }}"
                    ...
                    <span x-show="!sidebarMinimized" x-transition.opacity
                        class="truncate font-semibold tracking-wide">Oversight Training</span>
                </a>



                <p x-show="!sidebarMinimized" x-transition.opacity
                    class="px-3 pt-8 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 opacity-50">
                    Master Data</p>
                ...
                <a href="{{ route('admin.master-trainings.index') }}"
                    ...
                    <span x-show="!sidebarMinimized" x-transition.opacity
                        class="truncate font-semibold tracking-wide">Master Training</span>
                </a>
                ...
                <a href="{{ route('admin.employees.index') }}"
                    ...
                    <span x-show="!sidebarMinimized" x-transition.opacity
                        class="truncate font-semibold tracking-wide">Master Trainee</span>
                </a>
            </nav>
            ...
            <div class="border-t border-gray-200 dark:border-gray-800 p-4 transition-all duration-300">
                ...
                <div class="mb-4">
                    <button @click="toggleTheme()"
                        ...
                        :class="sidebarMinimized ? 'justify-center h-12' : 'px-3 py-3 gap-4'" title="Ganti Tema">
                        <div class="relative w-6 h-6 flex items-center justify-center shrink-0">
                            <!-- Sun Icon -->
                            <svg x-show="!darkMode"
                                class="w-5 h-5 transition-transform duration-500 group-hover:rotate-12" fill="none"
                ...
                <div class="flex items-center transition-all duration-300 overflow-hidden shrink-0"
                    ...
                    <div
                        class="w-10 h-10 bg-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold shrink-0 border border-indigo-500/20 shadow-inner">
                        {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                    </div>
                    <div x-show="!sidebarMinimized" x-transition.opacity
                        class="flex-1 min-w-0 overflow-hidden transition-all duration-300">
                        <p class="text-sm font-bold text-gray-900 dark:text-white truncate leading-tight mb-0.5">
                            {{ Auth::user()->name }}</p>
                        <p
                            class="text-[9px] text-gray-500 dark:text-gray-400 font-medium truncate uppercase tracking-tighter leading-tight">
                            {{ Auth::user()->email }}</p>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('logout') }}" class="mt-4">
                    ...
                    <button type="submit"
                        ...
                        :class="sidebarMinimized ? 'justify-center h-10' : 'px-3 py-2.5 gap-4'" title="Keluar">
                        <svg class="w-5 h-5 shrink-0 transition-transform group-hover/logout:-translate-x-1" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                        </svg>
                        <span x-show="!sidebarMinimized" x-transition.opacity
                            class="text-[10px] font-bold uppercase tracking-[0.25em] whitespace-nowrap leading-none">Keluar</span>
                    </button>
                </form>
            </div>
        </aside>
        ...
        <div class="flex-1 flex flex-col overflow-hidden bg-white dark:bg-gray-950">
            {{-- Top Bar --}}
            <header
                class="bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 h-28 flex-shrink-0 z-40 transition-colors duration-300">
                ...
                <div class="py-8 pl-8 md:pl-12 lg:pl-16 pr-8 md:pr-10 lg:pr-12 max-w-[1920px]">
                    @if (session('success'))
                        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                            ...
                            <div class="flex items-center gap-3">
                                <span
                                    class="flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-500/20 shadow-sm shadow-emerald-500/20">
                    ...
                    @if (session('error'))
                        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                            ...
                            <div class="flex items-center gap-3">
                                <span
                                    class="flex h-8 w-8 items-center justify-center rounded-xl bg-red-500/20 shadow-sm shadow-red-500/20">
                                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
